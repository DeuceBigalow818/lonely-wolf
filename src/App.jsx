...
function classifyTx(tx) {
  ...
  if (tx.contractAddress===CONTRACTS.SWAP) return "SWAP";
...
async function rpc(method,params={}) {
  ...
  try {
    ...
    console.log(`${tag} ✓`, typeof json.result === "string" ? json.result : "(object)");
    return json.result;
...
async function callClaude(system, user) {
  try {
    ...
    if (!r.ok) { console.error(`[Claude API] HTTP ${r.status}:`, await r.text().catch(()=>"")); return {error:true}; }
    const d = await r.json();
    const t = d.content?.find(c=>c.type==="text")?.text||"{}";
    return JSON.parse(t.replace(/```json|```/g,"").trim());
  } catch (e) { console.error("[Claude API] FAILED:", e.message || e); return {error:true}; }
}
...
async function verifyBTCPayment(txid) {
  try {
    ...
    // Optimistic: if mempool check fails (propagating), grant access
...
export default function LonelyWolf() {
  ...
  // ── Fetch BTC balance per wallet on load (batched to avoid RPC overload) ──
  ...
  function processBatch(txs) {
    ...
    if(newAl.length>0){
      ...
      if(fresh.length>0){
        ...
        if(np.length>0){const ex2=new Set(patsRef.current.map(p=>p.id));const fp=np.filter(p=>!ex2.has(p.id));if(fp.length>0){patsRef.current=[...fp,...patsRef.current].slice(0,50);setPatterns([...patsRef.current]);}}
      }
    }
  }
  ...
  async function analyzePattern(p){
    ...
    const pw=p.wallets.map(a=>{const w=wallets.find(x=>x.address===a);return w?`${w.label}(${w.tag},${w.alertCount}alerts)`:short(a);});
    const res=await callClaude(
      `You are a wolf pack behavior analyst for Bitcoin DeFi. Respond ONLY valid JSON.`,
  ...
  const txC={BURN:"#fc8181",DEPLOY:"#b794f4",STAKE:"#f6ad55",SWAP:"#68d391",TRANSFER:"#63b3ed",CONTRACT:"#fc8181",MOVE:"#2d3748"};
  ...
  const wolfStyles = `
    ...
    * { box-sizing:border-box; margin:0; padding:0; }
    ...
    .wolf-stars {
      ...
      background: radial-gradient(ellipse at 20% 50%,rgba(139,92,246,.04) 0%,transparent 60%),
    ...
    .scan-line {
      ...
      animation:scanLine 8s linear infinite;pointer-events:none;z-index:1;opacity:.4;
    }
    ...
    /* CARD */
    .wolf-card {
      background:rgba(13,11,18,.95);border:1px solid var(--border);border-radius:10px;
    ...
    /* INPUT */
    .wolf-inp {
      background:rgba(0,0,0,.6);border:1px solid rgba(139,92,246,.2);border-radius:6px;
      padding:8px 12px;color:var(--text);font-size:11px;font-family:'Crimson Pro',serif;outline:none;
    }
    .wolf-inp::placeholder{color:#2d3748;}
    ...
    .wolf-btn {
      ...
      background:rgba(139,92,246,.08);border:1px solid rgba(139,92,246,.25);color:var(--moon);
    }
    .wolf-btn:hover{background:rgba(139,92,246,.15);}
    .wolf-btn.ghost{background:transparent;border-color:rgba(255,255,255,.06);color:var(--fog);}
    .wolf-btn.danger{background:rgba(248,113,113,.08);border-color:rgba(248,113,113,.25);color:#fc8181;}
    ...
    .wolf-row:hover{background:rgba(139,92,246,.04);border-color:rgba(139,92,246,.15);}
    .wolf-row.sel{background:rgba(139,92,246,.08);border-color:rgba(139,92,246,.3);}

    /* PROGRESS */
    .wolf-prog{height:2px;border-radius:1px;background:rgba(255,255,255,.04);overflow:hidden;}
                        ...
                        <div style={{flex:1}}>
                          ...
                          <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                            {br.wolfTxs.slice(0,5).map((tx,j)=>{
                              const type=classifyTx(tx);
                              const tc={BURN:"#fc8181",DEPLOY:"#b794f4",STAKE:"#f6ad55",SWAP:"#68d391",TRANSFER:"#63b3ed",CONTRACT:"#fc8181",MOVE:"#6b7280"}[type]||"#6b7280";
                              return(
